...
<!-- ADD THIS NEW DROPDOWN DIRECTLY BESIDE IT: -->
<div>
  <label class="block font-semibold text-amber-700 mb-0.5 flex items-center gap-1">
    <i class="fa-solid fa-door-open text-amber-600"></i> Add Extra Room(s)
  </label>
  <select id="cust-extra-room" disabled onchange="calculateModalBilling()" class="w-full bg-amber-50 border border-amber-300 rounded-xl px-2.5 py-1.5 focus:outline-none focus:border-amber-500 font-bold text-amber-900 disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <option value="Same room no with extra bed">Same room no with extra bed</option>
        ...
        <div id="sec-billing-summary" class="bg-blue-50/40 p-3 rounded-2xl border border-blue-100 space-y-2.5 transition-all">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2 pb-2 border-b border-blue-200">
             <div>
   <label class="block font-semibold text-slate-600 mb-0.5">Extra Rate/Person (₹)</label>
   <input type="number" id="cust-extra-rate" value="1200" oninput="calculateModalBilling()" class="w-full bg-white font-bold text-amber-700 border border-amber-300 rounded-xl px-2 py-1.5 focus:outline-none focus:border-amber-500" />
</div>
             <div>
...
function updateDashboardCards() {
  ...
  // 1. POPULATE EXTRA ROOM DROPDOWN FROM MASTER TAB DATA
...
// 2. TOGGLE EXTRA PERSON / EXTRA ROOM FIELD STATES
function handleExtraPersonCountChange() {
  ...
  const timeWrapper = document.getElementById('sec-extra-person-time-wrapper');

  if (count > 0) {
    extraRoomSelect.disabled = false;
    timeWrapper.classList.remove('hidden');
  } else {
    // If set to 0 and no other extra field touched, reset
    const extraRoom = extraRoomSelect.value;
    const inDate = document.getElementById('cust-extra-person-date').value;
    const outDate = document.getElementById('cust-extra-person-out-date').value;
    
    if (!extraRoom && !inDate && !outDate) {
...
// 3. UPDATED MODAL BILLING CALCULATOR WITH EDITABLE EXTRA RATE
function calculateModalBilling() {
  ...
  // Calculate total extra charges based on editable rate
  const totalExtraPersonCharge = extraPersons * extraRate;

  // Store or process total extra charges
  document.getElementById('cust-extra-total')?.setAttribute('data-charge', totalExtraPersonCharge);

  const pricePerDay = parseFloat(document.getElementById('cust-price').value) || 0;
  const days = parseInt(document.getElementById('cust-days').value) || 1;
  const cabTotal = parseFloat(document.getElementById('cust-cab-total')?.value) || 0;
  
  // Update overall billing total including editable extra person rate
  const roomTotal = pricePerDay * days;
  const grandTotal = roomTotal + totalExtraPersonCharge + cabTotal;
  
  const advance = parseFloat(document.getElementById('cust-advance').value) || 0;
  const due = Math.max(0, grandTotal - advance);

  if (document.getElementById('cust-total')) document.getElementById('cust-total').value = grandTotal;
  if (document.getElementById('cust-due')) document.getElementById('cust-due').value = due;
}

// 4. VALIDATION ON BOOKING SAVE (Enforce strict extra person check)
function validateExtraPersonFields() {
  const count = parseInt(document.getElementById('cust-extra-persons').value) || 0;
  const extraRoom = document.getElementById('cust-extra-room').value.trim();
  const inDate = document.getElementById('cust-extra-person-date').value;
  const inTime = document.getElementById('cust-extra-person-time').value;
  const outDate = document.getElementById('cust-extra-person-out-date').value;
  const outTime = document.getElementById('cust-extra-person-out-time').value;

  const isAnyFilled = count > 0 || extraRoom !== "" || inDate !== "" || outDate !== "";

  if (isAnyFilled) {
    if (count <= 0 || extraRoom === "" || !inDate || !inTime || !outDate || !outTime) {
      alert("⚠️ INCOMPLETE EXTRA PERSON DETAILS:\n\nIf any extra person detail (Extra Count, Extra Room, or Check-In/Check-Out Dates) is entered, ALL extra person fields are required before saving!");
      return false;
    }
  }
  return true;
...
// 5. UPDATE IN HANDLE SAVE BOOKING
// In your existing handleSaveBooking(event) function, add this check before saving:
function handleSaveBooking(e) {
  e.preventDefault();
  
  if (!validateExtraPersonFields()) {
    return; // Block saving if extra person validation fails
  }
